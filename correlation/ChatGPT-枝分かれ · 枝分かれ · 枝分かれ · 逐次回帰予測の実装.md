# 枝分かれ · 枝分かれ · 枝分かれ · 逐次回帰予測の実装

**User:** Anonymous (kbmedphys@gmail.com)  
**Created:** 12/10/2025 19:48:43  
**Updated:** 12/10/2025 19:50:17  
**Exported:** 12/10/2025 20:00:03  
**Link:** [https://chatgpt.com/c/69395005-19f4-8323-ae25-e3ccf76e96a2](https://chatgpt.com/c/69395005-19f4-8323-ae25-e3ccf76e96a2)  
...
ざっくり言うと、**今のプログラム＋month\_type ラベリングでも「論文が言っている方向のシグナル」はかなり見えるはずだけれど、強さは本家よりやや弱く／時期によって劣化している**、という感じになると予想されます。

以下、先ほどお渡しした検証コードの各パートごとに「こういう結果が出るはず」という予想を整理します。

* * *

1\. basic\_stats\_method6：S\_t と x\_t の基本統計
-------------------------------------------

### 予想される結果

*   `df[["S", "x"]].describe()`
    *   **S\_t** は平均プラス寄り（好況期が多い）、分散はそこそこ大きい。
    *   **x\_t** は構造上「過去の S の平均を引いている」ので、**平均 ≈ 0** のほぼ中心化された分布。
*   Corr(S\_t, x\_t)
    *   x\_t は「S\_{t-1} − 過去平均S」を符号調整したものなので、**相関はかなり正**（0.7〜0.9 程度）になりやすい。
*   Corr(x\_t, r\_{t+1})
    *   フルサンプル（1926–2025）だと、**符号は正**で、値はそこまで大きくないが 0.1 前後の正相関が見える可能性が高い。
    *   これは論文で主張されている「z が将来の mkt を予測する」パターンと方向が一致します。
        ssrn-5268329

* * *

2\. regression\_predictability\_x：r\_{t+1} ~ x\_t
-------------------------------------------------
...
*   回帰：
    ...
    $$
*   フルサンプル 1926–2025 なら：
    *   **β > 0 で 5% 水準以上で有意**（t ≈ 2.5〜3.5）くらいは出ると期待できます。
    *   R² は **数％（1〜4% 程度）**。
        *   Guo & Wachter が報告している z\_{t-1} の OOS R² ≈ 4% と同じオーダー。
            ssrn-5268329
*   2020 年以降だけで同じ回帰を回すと：
    *   サンプルが短くなることもあり、**β の大きさ・有意性は大きく低下し、場合によっては統計的に 0** に近くなる可能性が高いです。
    *   ユーザーが既に観測されている「2020 年以降は予測力が弱い」という印象と整合的。

* * *

3\. regression\_S\_lag\_by\_monthtype：S\_{t−1} の符号反転仮定
------------------------------------------------------

ここは「Method 6 のロジック」を直接検証する部分です。

### 期待されるパターン

*   N 月（newsy）での回帰：
    $$
     r_t = \alpha_N + \beta_N S_{t-1} + \varepsilon_t
    $$
*   Non-N 月（R + 3）での回帰：
    $$
     r_t = \alpha_{O} + \beta_{O} S_{t-1} + \varepsilon_t
    $$

理論と論文の実証結果から期待されるのは：

*   **Non-N（特に R 月）では β\_O > 0（継続）**
*   **N 月では β\_N < 0（反転）**
    ssrn-5268329

ただし、現在の実装では：

*   month\_type が「カレンダー月 mod 3」で決まっており、
*   本家のように実際のアナウンス日（Compustat+IBES）に合わせていない、

ため、**符号のコントラスト（β\_N と β\_O の差）は論文より弱く、t 値も 2 前後に落ちる**イメージです。

それでも、

*   非 N 月側で β が 0 から有意に離れている（≒継続）
*   N 月側で β が負寄り（有意かどうかはギリギリ）

くらいは出ると予想されます。

* * *

4\. sort\_x\_quintiles：x\_t 分位での r\_{t+1}
-----------------------------------------

x\_t を 5 分位にソートして、翌月超過リターンの平均（年率）を見る部分です。

### 予想される形

*   Quintile 1（x が低い） → **将来リターンが高い**
*   Quintile 5（x が高い） → **将来リターンが低い（場合によっては負）**
*   1–5 スプレッド（Q1 − Q5）の年率で **5–10% 程度** の差が出る可能性がある。

これは直感的には：

*   x\_t が高い → 「Newsy 情報を既にかなり織り込んでいて割高」
*   x\_t が低い → 「割安／情報が十分織り込まれていない」

という mispricing の sign を表しているためで、Method 6 の「x は誤反応の蓄積をまとめた指標」という解釈と整合します。

* * *

5\. rolling\_corr\_x\_ret：x\_t と r\_{t+1} のローリング相関
--------------------------------------------------
...
*   1920s〜1970s:
    *   ローリング相関は **主に正**（たとえば +0.15〜+0.3 のレンジ）で推移。
*   1980s〜2000s 前半:
    *   ややボラティリティはあるが、 still positive on average。
*   2000s 後半〜2010s:
    *   相関が低下し、0 近辺をうろうろする期間が増える。
*   2020 年以降:
    *   非常に短いウィンドウでは有意な相関は出にくく、10 年ローリングでも
        *   2010–2020 頃は弱めの正相関、
        *   2015–2025 頃は 0 〜わずかな正に縮小  
            というイメージ。

つまりグラフとしては：

*   **長期的には常に「x と将来リターンの正の関係」は確認できるが、近年に向かうにつれてその強さが落ちてきている**
*   ユーザーが「2020 年以降では効きが弱い」と感じている印象は、このローリング相関でも視覚的に確認できるはずです。

* * *

6\. predictability\_by\_monthtype：月タイプ別の予測力
-------------------------------------------

x\_t の予測力を N / R / 3 別に見る回帰：

$$
 r_{t+1} = \alpha^{(k)} + \beta^{(k)} x_t + \varepsilon_{t+1},\quad k \in \{N,R,3\}
$$

### 予想される結果

*   **R 月**：
    *   β^(R) > 0 かつ 5% 水準で有意（t ≈ 2〜3）になりやすい
    *   継続パートに近い leg。
*   **N 月**：
    *   β^(N) > 0 だが t 値は 1〜2 程度でギリギリ〜非有意。
    *   反転 leg との兼ね合いで、符号は必ずしも強く出ない可能性もあり。
*   **3 月**：
    *   β^(3) はほぼ 0 に近く、有意性も低い。
    *   論文でも「third month はアナウンスが少なく、パターンが弱い」と述べられているので、これは自然な結果です。
        ssrn-5268329
...
現在のプログラムとラベル付け（calendar month mod 3）を前提にすると、検証プログラムから出てくるストーリーは：

1.  **S\_t, x\_t は理論通りの構造**
    *   x\_t はセンタリングされ、S\_t と強く結びついた「累積 new 情報の指標」として振る舞っている。
2.  **x\_t は 1926–2025 全体では、将来の市場超過リターン r\_{t+1} を統計的に有意に予測する**
    *   β > 0, t ≈ 3 前後, R² 数％。
3.  **S\_{t−1} と r\_t の関係は、Newsy と Non-newsy で符号が異なる（少なくとも方向性はそうなっている）**
    *   これにより「そのまま S を使うと予測方向がひっくり返る → sign-adjust + demean した x に落とし込む」という Method 6 の論理がデータでもサポートされる。
4.  **x\_t 級数でソートすると、x が高いほど翌月リターンが低く、低いほど高いという単調関係が見える**
    *   mispricing 指標としての経済的な意味づけが補強される。
5.  **ローリング相関では、長期には正の関係が安定しているが、直近（特に 2000 年代以降）で弱まっていることが見える**
    *   「構造としては存在するが、効き目は時期により変動している」という実務的なメッセージ。
6.  **月タイプ別では、Newsy / Repetitive が主戦場で、3 か月目はほぼノイズ**
    *   earnings cycle に紐づく現象であるという直感と一致。

* * *

もし「実際に手元で回してみた結果」と違うところがあれば、その差分が

*   month\_type ラベリングのずれ（カレンダー vs 実際の決算サイクル）
*   サンプル期間（1926–1960 を切るとどうなるか、post-2000 だけにしたらどうなるか）
*   Method 6 の x\_t 定義の細部（平均の取り方、符号など）

のどこから来ているのかまで一緒に詰めていけます。

次のステップとしては、

*   1926–1999 / 2000–2019 / 2020– などサブサンプル別に同じ検証を回す
*   その上で「2020 年以降低下」という現象を formal に図表化する

あたりを追加すると、「Method 6 の仮定自体は長期的には supported だが、近年は regime shift の兆候あり」というかなり強いストーリーまで持って行けると思います。
